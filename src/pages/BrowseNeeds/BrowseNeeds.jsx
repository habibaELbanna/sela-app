...
const BrowseNeeds = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const { data: needsData } = await supabase
        .from('needs')
        .select(
          `*, companies(name, logo_url, is_verified), categories(name_en, name_ar), governorates(name_en, name_ar)`
        )
        .eq('status', 'open')
  ...
  const getFilteredNeeds = () => {
    ...
    if (activeFilter === 'high_match')
      filtered = filtered.filter((n) => (matchScores[n.id] || 0) >= 80)
    ...
    >
      <div className='browse-header'>
        <div className='browse-header-top'>
          <img
            ...
            alt=''
            className='browse-logo-icon'
          />
          <img
            src={require('../../Assets/logoword.svg').default}
            alt='SELA'
            className='browse-logo-word'
          />
        </div>
      ...
      <div className='browse-list'>
        ...
        {filteredNeeds.map((need) => {
          ...
          const category = isAr
            ? need.categories?.name_ar
            : need.categories?.name_en
          const location = isAr
          ...
          return (
            ...
            >
              {closingSoon && (
                <div className='browse-badge-closing'>
              ...
              <div className='browse-card-top'>
                <div className='browse-company-row'>
                  {need.companies?.logo_url ? (
                    ...
                    <div className='browse-company-logo-placeholder' />
                  )}
              ...
              <h2 className='browse-card-title'>
                {isAr ? need.title_ar : need.title_en}
              </h2>

              {category && (
                <span className='browse-category-tag'>{category}</span>
              )}

              <div className='browse-card-meta'>
                <span className='browse-budget'>
                  {need.budget_min && need.budget_max_egp
                    ? `EGP ${need.budget_min.toLocaleString()} - ${need.budget_max_egp.toLocaleString()}`
                    : isAr
                      ? 'غير محدد'
                      : 'Budget TBD'}
                </span>
