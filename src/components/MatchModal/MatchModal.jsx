...
const MatchModal = ({ open, onClose, score, factors, entityName, entityType = 'vendor', isAr = false }) => {
  if (!open) return null

  const scoreClass = score >= 90 ? 'mm-score-high' : score >= 70 ? 'mm-score-mid' : 'mm-score-low'
  const scoreLabel = score >= 90
    ? (isAr ? 'تطابق ممتاز' : 'Excellent match')
    : score >= 70
    ? (isAr ? 'تطابق جيد' : 'Good match')
    : (isAr ? 'تطابق متوسط' : 'Moderate match')
  ...
  const renderFactorIcon = (status) => {
    ...
    if (status === 'partial') {
      return (
        <svg width='16' height='16' viewBox='0 0 24 24' fill='none'>
          ...
          <line x1='12' y1='7' x2='12' y2='13' stroke='#ffffff' strokeWidth='2.4' strokeLinecap='round'/>
    ...
    return (
      <svg width='16' height='16' viewBox='0 0 24 24' fill='none'>
        ...
        <line x1='7' y1='12' x2='17' y2='12' stroke='#ffffff' strokeWidth='2.4' strokeLinecap='round'/>
      </svg>
    )
  }

  return (
    <div className='mm-overlay' onClick={onClose} role='dialog' aria-modal='true' aria-label='Match explanation'>
      <div className='mm-sheet' onClick={(e) => e.stopPropagation()} dir={isAr ? 'rtl' : 'ltr'}>
        ...
        <div className='mm-header'>
          ...
          <div className='mm-header-text'>
            ...
            <span className='mm-subtitle'>{scoreLabel} · {entityName}</span>
          </div>
        ...
        <div className='mm-progress-wrap'>
          <div className='mm-progress-track'>
            <div className={`mm-progress-fill ${scoreClass}`} style={{ width: `${score}%` }} />
          </div>
            ...
            <div key={i} className={`mm-factor mm-factor-${f.status}`}>
              ...
              <div className='mm-factor-body'>
                <div className='mm-factor-top'>
                  <span className='mm-factor-name'>{isAr ? f.name_ar : f.name_en}</span>
                  <span className='mm-factor-weight'>+{f.weight}%</span>
                </div>
                <span className='mm-factor-detail'>{isAr ? f.detail_ar : f.detail_en}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className='mm-hint'>
          <svg width='14' height='14' viewBox='0 0 24 24' fill='none' stroke='#00a7e5' strokeWidth='2' strokeLinecap='round' strokeLinejoin='round'>
...
export const buildVendorFactors = (vendor, vp, gov, cat, buyerPrefs = {}, isAr = false) => {
  const factors = []
