...
export const ToastProvider = ({ children }) => {
  ...
  const success = useCallback(
    (msg, duration) => show(msg, 'success', duration),
    [show]
  )
  const error = useCallback(
    (msg, duration) => show(msg, 'error', duration),
    [show]
  )
  const info = useCallback(
    (msg, duration) => show(msg, 'info', duration),
    [show]
  )

  const lang =
    typeof window !== 'undefined'
      ? localStorage.getItem('sela_lang') || 'en'
      : 'en'
  const isAr = lang === 'ar'
      ...
      return (
        <svg width='18' height='18' viewBox='0 0 24 24' fill='none'>
          ...
          <line
            ...
            strokeLinecap='round'
          />
          <circle cx='12' cy='16' r='1' fill='#cccccc' />
        </svg>
      )
    return (
      <svg width='18' height='18' viewBox='0 0 24 24' fill='none'>
