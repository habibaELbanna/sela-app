...
const BrowseNeeds = () => {
    ...
    const fetchData = async () => {
      ...
      if (scoresData) {
        ...
        scoresData.forEach((s) => {
          if (
            !scoresMap[s.need_id] ||
            s.score_percentage > scoresMap[s.need_id]
          ) {
  ...
  const handleSave = async (e, needId) => {
    ...
    setTimeout(
      () => setSavedAnim((prev) => ({ ...prev, [needId]: false })),
      400
    )
        ...
        <div className='browse-list'>
          ...
          {filteredNeeds.map((need) => {
            ...
            const companyName = isAr
              ? need.companies?.name_ar
              : need.companies?.name_en
            const category = isAr
            ...
            return (
              <div
                key={need.id}
                className={`browse-card ${closingSoon ? 'browse-card-has-badge' : ''}`}
                onClick={() => navigate(`/need/${need.id}`)}
              >
                {closingSoon && (
                  <img
                    src={require('../../Assets/badge.svg').default}
                    alt='Closing Soon'
                    className='browse-badge-closing'
                  />
                )}
                ...
                <div className='browse-card-top'>
                  <div className='browse-company-row'>
                    ...
                    <div
                      ...
                      style={{
                        display: need.companies?.logo_url ? 'none' : 'flex',
                      }}
                    >
